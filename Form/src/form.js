...
function MyForm() {
  ...
  const[pref,setPref] = useState("orange");
  const handleSubmit = (event) => {
    event.preventDefault();
    // alert(`The inputs were: ${name} ${number} ${email} ${pref}`);
  }
  
  return (
    
    <form class="form-control" onSubmit={handleSubmit}>
        <h1 class="text-center text-5xl font-bold font-serif">PressReads</h1>
        <p class="text-center mt-2">Stay up to date with our latest news and products...</p>
      <fieldset>  
